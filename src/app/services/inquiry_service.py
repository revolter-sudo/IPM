...
def create_inquiry_service(
    ...
    try:
        ...
        return InquiryServiceResponse(
            data=None, message=f"Error creating inquiry: {str(e)}", status_code=500
        )
...
def get_inquiries_service(
    ...
    try:
        ...
        inquiries = (
            query.order_by(InquiryData.created_at.desc())
            .offset(offset)
            .limit(page_size)
            .all()
        )
        ...
        return InquiryServiceResponse(
            data=None, message=f"Error fetching inquiries: {str(e)}", status_code=500
        )


def get_inquiry_by_uuid_service(
    db: Session, inquiry_uuid: str
) -> InquiryServiceResponse:
    """
    ...
    try:
        inquiry = (
            ...
            .filter(
                and_(
                    InquiryData.uuid == inquiry_uuid, InquiryData.is_deleted.is_(False)
                )
            )
            .first()
        )
        ...
        if not inquiry:
            return InquiryServiceResponse(
                data=None, message="Inquiry not found", status_code=404
            )
        ...
        return InquiryServiceResponse(
            data=None, message=f"Error fetching inquiry: {str(e)}", status_code=500
        )
