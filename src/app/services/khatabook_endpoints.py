...
from src.app.database.models import Khatabook, KhatabookBalance, User
from src.app.schemas import constants
from src.app.schemas.auth_service_schamas import AuthServiceResponse, UserRole
from src.app.schemas.khatabook_schemas import (
    KhatabookServiceResponse,
...
async def create_khatabook_entry(
    ...
    try:
        ...
        return AuthServiceResponse(
            data=None, status_code=500, message=f"Error: {str(e)}"
        ).model_dump()
...
def update_khatabook_entry(
    ...
    try:
        parsed_data = json.loads(data)
        file_paths = [save_uploaded_file(f) for f in files] if files else []
        entry = update_khatabook_entry_service(
            db, khatabook_uuid, parsed_data, file_paths
        )
        if not entry:
            return AuthServiceResponse(
...
def get_all_khatabook_entries(
    ...
    try:
        ...
        user_balance_record = (
            ...
            .filter(KhatabookBalance.user_uuid == current_user.uuid)
            .first()
        )
...
def mark_suspicious(
    ...
    try:
        ...
        entry = mark_khatabook_entry_suspicious(
            db=db, kb_uuid=khatabook_uuid, is_suspicious=request.is_suspicious
        )
    ...
    try:
        ...
        if is_admin and has_filters:
            ...
            query = (
                ...
                .outerjoin(
                    User, Khatabook.created_by == User.uuid
                )  # Uncommented User join
                .outerjoin(Person, Khatabook.person_id == Person.uuid)
                .outerjoin(
                    Project, Khatabook.project_id == Project.uuid
                )  # Uncommented Project join
            ...
            if start_date:
                try:
                    ...
                    query = query.filter(Khatabook.expense_date >= start_date_obj)
                except ValueError:
                    return KhatabookServiceResponse(
                        data=None,
                        status_code=400,
            ...
            if end_date:
                try:
                    ...
                    query = query.filter(Khatabook.expense_date <= end_date_obj)
                except ValueError:
                    return KhatabookServiceResponse(
                        data=None,
                        status_code=400,
            ...
            khatabook_entries = query.options(
                ...
                if entry.items:
                    for khatabook_item in entry.items:
                        if khatabook_item.item:
                            items_data.append(
                                {"item": {"name": khatabook_item.item.name}}
                            )

                # Process person info
                person_info = None
                if entry.person:
                    person_info = {"name": entry.person.name}

                # Process created_by user info
                created_by_info = None
                if entry.created_by_user:
                    created_by_info = {"name": entry.created_by_user.name}

                entries.append(
                    {
                        "created_at": entry.created_at.isoformat(),
                        "expense_date": (
                            entry.expense_date.isoformat() if entry.expense_date else ""
                        ),
                        "amount": entry.amount,
        ...
        for entry in entries:
            person_name = (
                ...
                if isinstance(entry.get("person"), dict)
                else ""
            )
