...
def upgrade():
    ...
    op.add_column(
        "person", sa.Column("upi_number", sa.String(length=10), nullable=True)
    )

    # 2) Alter columns to make them nullable
    op.alter_column(
        "person", "account_number", existing_type=sa.String(length=17), nullable=True
    )
    op.alter_column(
        "person", "ifsc_code", existing_type=sa.String(length=11), nullable=True
    )
