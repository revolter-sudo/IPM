...
try:
    ...
    print(
        "Error: Could not import application models. Make sure you're running this script from the project root."
    )
    sys.exit(1)
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("upload_fix.log"), logging.StreamHandler()],
)
    ...
    for user in users:
        ...
        if "uploads/payments/users/" in user.photo_path:
            ...
            if not os.path.exists(file_path):
                logging.warning(
                    f"User {user.uuid}: Photo file not found at {file_path}"
                )
                issues += 1
...
def check_payment_files():
    ...
    payment_files = (
        session.query(PaymentFile).filter(PaymentFile.is_deleted.is_(False)).all()
    )
...
def check_invoice_files():
    ...
    for invoice in invoices:
        ...
        if not os.path.exists(invoice.file_path):
            logging.warning(
                f"Invoice {invoice.uuid}: File not found at {invoice.file_path}"
            )
            issues += 1
...
def check_po_documents():
    ...
    for project in projects:
        ...
        if not os.path.exists(project.po_document_path):
            logging.warning(
                f"Project {project.uuid}: PO document not found at {project.po_document_path}"
            )
            issues += 1
...
def main():
    ...
    if total_issues > 0:
        ...
        logging.info(
            "No issues found. All files appear to be in the correct locations."
        )
