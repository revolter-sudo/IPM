...
TEST_PROJECT_DATA = {
    ...
    "pos": [
        ...
        {"po_number": "PO002", "amount": 1000.0, "description": "Second PO for labor"},
    ],
}
...
class TestImplementation:
    ...
    def test_project_creation_with_multiple_pos(self):
        ...
        try:
            # Prepare form data
            form_data = {"request": json.dumps(TEST_PROJECT_DATA)}
            ...
            for i in range(2):
                ...
                dummy_content = (
                    f"Dummy PO Document {i+1}\nPO Number: PO00{i+1}\nAmount: 1000.0"
                )
                files[f"po_document_{i}"] = (
                    f"po_document_{i}.txt",
                    dummy_content,
                    "text/plain",
                )

            response = self.session.post(
                f"{BASE_URL}/projects/create", data=form_data, files=files
            )
    ...
    def test_invoice_upload_with_po_link(self):
        ...
        try:
            ...
            # Prepare form data
            form_data = {"request": json.dumps(invoice_data)}

            # Create dummy invoice file
            dummy_invoice = (
                "Dummy Invoice Document\nAmount: 750.0\nDue Date: 2025-06-15"
            )
            files = {"invoice_file": ("invoice.txt", dummy_invoice, "text/plain")}

            response = self.session.post(
                f"{ADMIN_BASE_URL}/invoices", data=form_data, files=files
            )
        ...
        try:
            ...
            if response1.status_code == 201:
                ...
                if response2.status_code == 201:
                    ...
                    print(
                        f"   Invoice status: {result2['data']['invoice_payment_status']}"
                    )
                    return True
                else:
    ...
    def run_all_tests(self):
        ...
        tests = [
            ...
            (
                "Project Creation with Multiple POs",
                self.test_project_creation_with_multiple_pos,
            ),
